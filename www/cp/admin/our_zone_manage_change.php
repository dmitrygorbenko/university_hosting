<?php
	...
	function body_page() {
		...
		$subdomains = $HM->ZC->fetch_all_subdomains_of_our_zone($id_zone);
		if ($subdomains == FALSE) {
			echo "Can't fetch all subdomains of our zone: ".$_SESSION["adminpanel_error"];
			return;
		}

		$zone_info = $HM->ZC->fetch_our_zone_info($id_zone);
		if ($zone_info == FALSE) {
			echo "Can't fetch our zone info: ".$_SESSION["adminpanel_error"];
			return;
		}
		...
		<table cellspacing=\"3\" cellpadding=\"0\" border=\"0\" width=\"100%\">
			...
			<tr valign=\"top\">
				<td colspan=\"6\">
					<input type=\"button\" class=\"simple_button\" onClick=\"document.location='our_zone_manage_change_manually.php?id_zone=".$id_zone."'\"\" value=\"Manual Mode\">
				</td>
			</tr>
			<tr valign=\"top\">
		...
		for ($i = 0; $i < count($zone_info); $i++, $index++)
			echo	"
			<tr valign=\"top\">
				<form action=\"our_zone_manage_action.php\" method=\"POST\" id=\"modify_d_mx_".$i."\" name=\"modify_d_mx_".$i."\">
		...
		$index = 1;
			...
			echo "
			<tr valign=\"top\">
				<form action=\"our_zone_manage_action.php\" method=\"POST\" id=\"create_d_mx\" name=\"create_d_mx\">
				<input type=\"hidden\" name=\"id_zone\" value=\"".$id_zone."\">
		...
		for ($i = 0; $i < $HM->ZC->SQL->get_num_rows($subdomains); $i++) {
			$data = $HM->ZC->SQL->get_object($subdomains);

			echo "
				<tr valign=\"top\">
					<form action=\"our_zone_manage_action.php\" method=POST id=\"modify_record_".$i."\" name=\"modify_record_".$i."\">
					<input type=\"hidden\" name=\"id_zone\" value=\"".$id_zone."\">
		...
		echo "
			<tr valign=\"top\">
				<form action=\"our_zone_manage_action.php\" method=POST id=\"create_record\" name=\"create_record\">
				<input type=\"hidden\" name=\"id_zone\" value=\"".$id_zone."\">
