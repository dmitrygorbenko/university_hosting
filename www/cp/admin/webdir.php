<?php
	...
	function body_page() {
		...
		if ($id_client == "" || $id_client == "none") {
			$zones = $HM->ZC->fetch_all_zones_for_webdir();
			if ($zones == FALSE) {
			...
			$zones = $HM->ZC->fetch_all_zones_of_client_for_webdir($id_client);
			if ($zones == FALSE) {
		...
		$webdir_count = $HM->WC->get_webdirs_count();
		if ($webdir_count == FALSE) {
			echo "Can't get webdirs count: ".$_SESSION["adminpanel_error"];
			return;
		}

		$alone_webdir_count = $HM->WC->get_alone_webdirs_count();
		if ($alone_webdir_count == FALSE) {
			echo "Can't get alone webdirs count: ".$_SESSION["adminpanel_error"];
			return;
		}
		...
		echo "
				...
				<td colspan=\"4\" width=\"100%\">
					...
					<tr valign=\"center\" height=\"62\" width=\"100%\">
						...
						<td height=\"62\" width=\"99%\" align=\"left\" background=\"../img/table/table_background.jpg\" class=\"title\" colspan=\"3\">
							Web Dir Manage: Select Hosting Zone
						</td>
			...
			<tr valign=\"top\">
				<td width=\"100%\" colspan=\"4\">
					Client:
					&nbsp;&nbsp;&nbsp;&nbsp;
					<select id=\"client_select\" name=\"id_client\" class=\"simple_select\" onChange=\"update_page('webdir.php', 'client_select', 'id_client')\">
		...
		echo "
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					<a href=\"webdir_select.php?id_zone=none\" title=\"View All Dirs\">View All Dirs (".$webdir_count->webdir_count.")</a>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<a href=\"webdir_select.php?id_zone=alone\" title=\"View non-Client Dirs\">non-Client Dirs (".$alone_webdir_count->webdir_count.")</a>
				</td>
			</tr>
			<tr valign=\"top\">
				...
				<td width=\"13%\" class=\"content\">Web Dirs Count</td>
			</tr>
			";
		...
		for ($i = 0; $i < $HM->WC->SQL->get_num_rows($zones); $i++) {
			...
			echo "<tr>
				...
				<td class=\"content_3\"><a href=\"webdir_select.php?id_zone=".$data->id_table."\">".$data->name."</a></td>
				<td class=\"content_3\"><a href=\"client_change.php?id=".$data->id_client_table."\">".$data->client."</a></td>
				<td class=\"content_3\">".$data->webdir_count."</td>
			</tr>";
			$zones_empty = FALSE;
		}
