	...
	if ($manual == "") {
		...
		if ($action == "create_record") {
			...
			if ($domain != "" && $type != "" && $prior != "" && $record != "") {
				if ($HM->SDC->create_domain_record($zone_data->id_table, $zone_data->name, $domain, $type, $prior, $record) != TRUE) {
		...
		if ($action == "modify_record") {
			...
			if ($id_domain != "" && $old_domain != "" && $old_type != "" && $old_prior != "" && $domain != "" && $type != "" && $prior != "" && $record != "") {
				if ($HM->SDC->update_domain_record($zone_data->id_table, $zone_data->name, $id_domain, $old_domain, $old_type, $old_prior, $domain, $type, $prior, $record) != TRUE) {
		...
		if ($action == "delete_record") {
			...
			if ($id_domain != "" && $old_domain != "" && $old_type != "" && $old_prior != "") {
				if ($HM->SDC->remove_domain_record($zone_data->name, $id_domain, $old_domain, $old_type, $old_prior) != TRUE) {
		...
		if ($HM->SDC->remove_all_subdomains_of_zone_in_db($id_zone) != TRUE) {
		...
		for ($i = 0; $i < count($records); $i++) {
			if ($HM->SDC->add_subdomain_in_db($id_zone, $records[$i]["domain"], strtoupper($records[$i]["type"]), $records[$i]["prior"], $records[$i]["record"]) != TRUE) {
		...
		if ($HM->SDC->change_subdomain_manually($zone_data->name, $zone_content) != TRUE) {
