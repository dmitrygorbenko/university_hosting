<?php
	...
	function body_page() {
		...
		if ($id_client == "" || $id_client == "none") {
			$zones = $HM->ZC->fetch_all_zones_for_ftp();
			if ($zones == FALSE) {
			...
			$zones = $HM->ZC->fetch_all_zones_of_client_for_ftp($id_client);
			if ($zones == FALSE) {
		...
		$ftp_count = $HM->FC->get_ftps_count();
		if ($ftp_count == FALSE) {
			echo "Can't get ftps count: ".$_SESSION["adminpanel_error"];
			return;
		}

		$alone_ftp_count = $HM->FC->get_alone_ftps_count();
		if ($alone_ftp_count == FALSE) {
			echo "Can't get alone ftps count: ".$_SESSION["adminpanel_error"];
			return;
		}
		...
		echo "
			...
			<tr valign=\"top\">
				<td width=\"100%\" colspan=\"4\">
					Client:
					&nbsp;&nbsp;&nbsp;&nbsp;
					<select id=\"client_select\" name=\"id_client\" class=\"simple_select\" onChange=\"update_page('ftp.php', 'client_select', 'id_client')\">
		...
		echo "
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					<a href=\"ftp_select.php?id_zone=none\" title=\"View All FTPs\">View All FTPs (".$ftp_count->ftp_count.")</a>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<a href=\"ftp_select.php?id_zone=alone\" title=\"View non-Client FTPs\">non-Client FTPs (".$alone_ftp_count->ftp_count.")</a>
				</td>
			</tr>
			<tr valign=\"top\">
		...
		for ($i = 0; $i < $HM->ZC->SQL->get_num_rows($zones); $i++) {
			...
			echo "<tr>
				...
				<td class=\"content_3\"><a href=\"ftp_select.php?id_zone=".$data->id_table."\">".$data->name."</a></td>
				<td class=\"content_3\"><a href=\"client_change.php?id=".$data->id_client_table."\">".$data->client."</a></td>
				<td class=\"content_3\">".$data->ftp_count."</td>
			</tr>";
			$zones_empty = FALSE;
		}
