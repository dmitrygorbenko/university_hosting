<?php
	...
	function body_page() {
		...
		if ($id_client == "" || $id_client == "none") {
			$zones = $HM->ZC->fetch_all_zones_for_mail_forwarders();
			if ($zones == FALSE) {
			...
			$zones = $HM->ZC->fetch_all_zones_of_client_for_mail_forwarders($id_client);
			if ($zones == FALSE) {
		...
		$forwarder_count = $HM->MC->get_email_forwarders_count();
		if ($forwarder_count == FALSE) {
			echo "Can't get email forwarder count: ".$_SESSION["adminpanel_error"];
			return;
		}

		$alone_forwarder_count = $HM->MC->get_alone_email_forwarders_count();
		if ($alone_forwarder_count == FALSE) {
			echo "Can't get alone email forwarder count: ".$_SESSION["adminpanel_error"];
			return;
		}
		...
		echo "
				...
				<td colspan=\"4\" width=\"100%\">
					...
					<tr valign=\"center\" height=\"62\" width=\"100%\">
						...
						<td height=\"62\" width=\"99%\" align=\"left\" background=\"../img/table/table_background.jpg\" class=\"title\" colspan=\"3\">
							E-Mail Forwarder Manage: Select Hosting Zone
						</td>
			...
			<tr valign=\"top\">
				<td width=\"100%\" colspan=\"4\">
					Client:
					&nbsp;&nbsp;&nbsp;&nbsp;
					<select id=\"client_select\" name=\"id_client\" class=\"simple_select\" onChange=\"update_page('mail_forwarder.php', 'client_select', 'id_client')\">
		...
		echo "
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					<a href=\"mail_forwarder_select.php?id_zone=none\" title=\"View All E-Mail Forwarders\">View All E-Mail Forwarders (".$forwarder_count->forwarders_count.")</a>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<a href=\"mail_forwarder_select.php?id_zone=alone\" title=\"View non-Client E-Mail Forwarders\">non-Client E-Mail Forwarders (".$alone_forwarder_count->forwarders_count.")</a>
				</td>
			</tr>
			<tr valign=\"top\">
		...
		for ($i = 0; $i < $HM->ZC->SQL->get_num_rows($zones); $i++) {
			...
			echo "<tr>
				...
				<td class=\"content_3\"><a href=\"mail_forwarder_select.php?id_zone=".$data->id_table."\">".$data->name."</a></td>
				<td class=\"content_3\"><a href=\"client_change.php?id=".$data->id_client_table."\">".$data->client."</a></td>
				<td class=\"content_3\">".$data->forwarder_count."</td>
			</tr>";
			$zones_empty = FALSE;
		}
