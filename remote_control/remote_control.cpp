...
void Signal_TERM(int sig)
{
	if (sig == SIGINT)
		...
		fprintf(stderr, "Cought SIGHUP\n");
	else if (sig == SIGTERM)
		fprintf(stderr, "Cought SIGTERM\n");
	else if (sig == SIGSEGV)
		fprintf(stderr, "Cought SIGSEGV\n");
...
unsigned int become_root()
{
	...
	if (effective != 0) {
		fprintf(stderr, "File haven't '+s' flag or owner is not 'root'\n");
		return FALSE;
	}
...
int main(int argc, char *argv[])
{
	...
	if (rr == -1) {
		fprintf(stderr, "listenfd reuseaddr failed\n");
	}
